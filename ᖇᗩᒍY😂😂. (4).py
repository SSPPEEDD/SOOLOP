...
g= datetime.datetime(2023, 12, 30, 00, 00 ,0) #هنا تقدر تغير الوقت بالأداة

if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("     "+' انتهت الصلاحيه عليك بمراسله المطورللحصول على احدث نسخه @XD_0O ' )
 print('\n\n')
 print(x)
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠘⢮⡇⠀⠀⠈⠓⠃⠀⠀⠀⠈⠀⠀⠀⠀
""")
	print(Z+'\x1b[38;5;208mTelegram & @V4V4J  ')
	print(F+          'مدفوعه')
	print('[@V4V4J]')
	print(f'»  Your ID&ايديك : '+str(my_id))
	print(f'»  Your IP&ايبي جهازك : {ip}')
	print('')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s <@V4V4J> %s/%s » [OK] %s » [CP] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
SPED  -  @V4V4J				
					...
					cetak(nel(statusok1, title=' صحيح '))
